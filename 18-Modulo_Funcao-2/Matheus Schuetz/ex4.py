...
def calc_previdencia(salario):
    if salario > 0 and salario <= 1000:
        resultado = (salario * 0.01) 
    elif salario >= 1000.01 and salario <= 3000:
        resultado = (salario * 0.015) 
    elif salario >= 3000.01 and salario <= 6000:
        resultado = (salario * 0.02) 
    else:
        resultado = (salario * 0.025) 
    return resultado


salario = float(input('Digite seu salário para saber o desconto da previdencia:'))
resultado = calc_previdencia(salario)
print(f'O valor do desconto será de {resultado}')
